...
public class BoardTeam {
	...
	public static void main(String[] args) {
		...
		// string 객체 선언
		String[][] myBoard;
		// 게시판 목록수 : boardNum개 게시판 항목수 : 100개
		...
		while (run) {
					...
					for (int i = count; i > 0; i--) {// 최근 게시물 번호부터 시작해서 1까지
						for (int j = 0; j < myBoard.length; j++) { // 0부터 boardNum개: 최대 길이까지 확인용
							if (myBoard[j][0] != null && myBoard[j][0].equals("" + i)) {
								System.out.println(myBoard[j][0] + "\t" + myBoard[j][1] + "\t" + myBoard[j][2] + "\t"
										+ myBoard[j][3] + "\t" + myBoard[j][4]);
							} 
						}
					}
				}
			 ...
			 */
				...
				int tempHit = Integer.parseInt(myBoard[selectIndex][4]) + 1; // 조회수 1 증가
